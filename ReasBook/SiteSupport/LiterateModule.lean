...
where
  ...
  normalizeInternalHref (href : String) : String :=
    ...
    else if href.startsWith "http://" || href.startsWith "https://" then href
    else if href.startsWith "mailto:" || href.startsWith "tel:" then href
    else "/" ++ href
  ...
  | .a href _title _isAuto contents => do
    let href := (← href.mapM ofAttrText).toList |> String.join |> normalizeInternalHref
...
open PartElabM in
def elabReasBookPage (x : Ident) (mod : Ident) (config : LitPageConfig) (title : StrLit) (genre : Term) (metadata? : Option Term) (rw : Option (TSyntax ``rewrites)) : CommandElabM Unit :=
...
/--
Creates a ReasBook page from a literate Lean module with Markdown module docstrings in it, performing a
...
-/
syntax "reasbook_page " ident optConfig " from " ident "  as " str (" with " term)? (rewrites)? : command
